...
- Status: blocked until hosted API health is repaired.
- Risk: Railway API `/health` previously failed with `ETIMEDOUT`, likely hosted Postgres/runtime connectivity or expensive health reads.
