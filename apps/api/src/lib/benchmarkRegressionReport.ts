import { benchmarkRegressionReportSchema } from "@finance-superbrain/schemas";
import type { BenchmarkRegressionReport, BenchmarkTrendReport } from "@finance-superbrain/schemas";

import { buildBenchmarkTrendReport } from "./benchmarkTrendReport.js";
import type { Repository } from "./repository.types.js";

type Severity = BenchmarkRegressionReport["alerts"][number]["severity"];
...
const calculateRegressionStreak = (snapshots: BenchmarkTrendReport["families"][number]["snapshots"]) => {
...
export const buildBenchmarkRegressionReport = async (
  repository: Repository,
  options: {
    benchmark_pack_id?: string;
    limit?: number;
  } = {},
): Promise<BenchmarkRegressionReport> => {
  const trend = await buildBenchmarkTrendReport(repository, options);
  return buildBenchmarkRegressionReportFromTrend(trend);
};

export const buildBenchmarkRegressionReportFromTrend = (
  trend: BenchmarkTrendReport,
  ...
  const alerts = trend.families
    .map((family) => {
      ...
      if (scoreDelta <= -0.03) {
        signals.push(`core benchmark score trails the family baseline by ${scoreDelta}`);
      }

      if (directionDelta <= -0.05) {
        signals.push(`core benchmark direction accuracy trails the family baseline by ${directionDelta}`);
      }

      if (wrongRateDelta >= 0.05) {
        signals.push(`core benchmark wrong rate exceeds the family baseline by ${wrongRateDelta}`);
      }

      if (calibrationGapDelta >= 0.05) {
        signals.push(
          `core benchmark calibration gap is wider than the family baseline by ${calibrationGapDelta}`,
        );
      }

      if (regressionStreak >= 2) {
        signals.push(
          `core benchmark regression has persisted for ${regressionStreak} consecutive checkpoints`,
        );
      }
      ...
      return {
        ...
        recommended_action:
          severity === "high"
            ? `Treat ${family.family} as real growth pressure and queue diagnostics before the next molt review.`
            : severity === "medium"
              ? `Keep ${family.family} under benchmark watch and rerun replay diagnostics soon.`
              : `Monitor ${family.family}; the mixed benchmark is softening but not yet critical.`,
      };
    })
    .filter((item): item is BenchmarkRegressionReport["alerts"][number] => item !== null)
    .sort((left, right) => {
  ...
  return benchmarkRegressionReportSchema.parse({
