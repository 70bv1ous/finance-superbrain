import { walkForwardRegimeRegressionReportSchema } from "@finance-superbrain/schemas";
import type {
  WalkForwardRegimeRegressionReport,
...
type Severity = WalkForwardRegimeRegressionReport["alerts"][number]["severity"];
...
const calculateRegressionStreak = (
  snapshots: WalkForwardRegimeTrendReport["slices"][number]["snapshots"],
) => {
...
export const buildWalkForwardRegimeRegressionReport = async (
  repository: Repository,
  options: {
    benchmark_pack_id?: string;
    limit?: number;
  } = {},
): Promise<WalkForwardRegimeRegressionReport> => {
  const trend = await buildWalkForwardRegimeTrendReport(repository, options);
  return buildWalkForwardRegimeRegressionReportFromTrend(trend);
};

export const buildWalkForwardRegimeRegressionReportFromTrend = (
  trend: WalkForwardRegimeTrendReport,
): WalkForwardRegimeRegressionReport => {
  const alerts = trend.slices
    .map((slice) => {
      const previousSnapshots = slice.snapshots.slice(0, -1);

      if (!previousSnapshots.length) {
        return null;
      }

      const latest = slice.snapshots[slice.snapshots.length - 1]!;
      const strongestPriorScore = previousSnapshots.reduce(
        (max, snapshot) => Math.max(max, snapshot.average_total_score),
        previousSnapshots[0]!.average_total_score,
      );
      ...
      const regressionStreak = calculateRegressionStreak(slice.snapshots);
      const signals: string[] = [];

      if (scoreDelta <= -0.03) {
        signals.push(
          `walk-forward ${slice.regime} score trails the family regime baseline by ${scoreDelta}`,
        );
      }

      if (directionDelta <= -0.05) {
        signals.push(
          `walk-forward ${slice.regime} direction accuracy trails the family regime baseline by ${directionDelta}`,
        );
      }

      if (wrongRateDelta >= 0.05) {
        signals.push(
          `walk-forward ${slice.regime} wrong rate exceeds the family regime baseline by ${wrongRateDelta}`,
        );
      }

      if (calibrationGapDelta >= 0.05) {
        signals.push(
          `walk-forward ${slice.regime} calibration gap is wider than the family regime baseline by ${calibrationGapDelta}`,
        );
      }

      if (regressionStreak >= 2) {
        signals.push(
          `walk-forward ${slice.regime} regression has persisted for ${regressionStreak} consecutive checkpoints`,
        );
      }
      ...
      return {
        ...
        model_version: slice.latest_model_version,
        severity,
        ...
        recommended_action:
          severity === "high"
            ? `Treat ${slice.family} as weak in ${slice.regime} conditions and strengthen diagnostics or shell prep around that regime before hardening again.`
            : severity === "medium"
              ? `Keep ${slice.family} under timed watch in ${slice.regime} conditions and prioritize more memory or diagnostics there.`
              : `Monitor ${slice.family} in ${slice.regime}; timed performance is softening but not yet critical.`,
      };
    })
    .filter((item): item is WalkForwardRegimeRegressionReport["alerts"][number] => item !== null)
    .sort((left, right) => {
      if (severityRank[right.severity] !== severityRank[left.severity]) {
        return severityRank[right.severity] - severityRank[left.severity];
      }

      if (right.regression_streak !== left.regression_streak) {
        return right.regression_streak - left.regression_streak;
      }
  ...
  return walkForwardRegimeRegressionReportSchema.parse({
    generated_at: new Date().toISOString(),
    benchmark_pack_id: trend.benchmark_pack_id,
    regime_count: new Set(alerts.map((alert) => alert.regime)).size,
    counts: {
