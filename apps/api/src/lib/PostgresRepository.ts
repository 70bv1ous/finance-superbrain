...
export class PostgresRepository implements Repository {
  ...
  private async getSharedInvestigationSteps(investigationId: string) {
    ...
    return query.rows.map((row) => mapSharedInvestigationStepRow(row as DbRow));
  }
  ...
  async getOrCreateDefaultWorkspace(): Promise<Workspace> {
    ...
    if (existing.rowCount) {
      return mapWorkspaceRow(existing.rows[0] as DbRow);
    }
  ...
  async countWorkspaceUsers(): Promise<number> {
    const query = await this.pool.query(`select count(*)::int as count from workspace_users`);
  ...
  async getWorkspaceUserById(id: string) {
    ...
    return query.rowCount ? mapWorkspaceUserRow(query.rows[0] as DbRow) : null;
  }
  ...
  async getWorkspaceMembership(input: { workspace_id: string; user_id: string }) {
    ...
    return query.rowCount ? mapWorkspaceMembershipRow(query.rows[0] as DbRow) : null;
  }
  ...
  async getUserSessionByTokenHash(tokenHash: string) {
    ...
    return query.rowCount ? mapAuthSessionRow(query.rows[0] as DbRow) : null;
  }
  ...
  async getServerStudioDraft(input: { workspace_id: string; owner_user_id: string }) {
    ...
    return query.rowCount ? mapServerStudioDraftRow(query.rows[0] as DbRow) : null;
  }
  ...
  async getSharedStudioRun(id: string) {
    const query = await this.pool.query(`select * from shared_studio_runs where id = $1 limit 1`, [id]);
    return query.rowCount ? mapSharedStudioRunRow(query.rows[0] as DbRow) : null;
  }
  ...
  async listSharedStudioRuns(options: { workspace_id: string; owner_user_id?: string; limit?: number }) {
    ...
    return query.rows.map((row) => mapSharedStudioRunRow(row as DbRow));
  }
  ...
  async replaceSharedInvestigationSteps(input: { investigation_id: string; steps: SharedInvestigation["steps"] }) {
    await this.pool.query(`delete from shared_investigation_steps where investigation_id = $1`, [input.investigation_id]);
  ...
  async getSharedInvestigation(id: string) {
    const query = await this.pool.query(`select * from shared_investigations where id = $1 limit 1`, [id]);
    if (!query.rowCount) {
      return null;
    }
  ...
  async listSharedInvestigations(options: {
    ...
    return Promise.all(
      query.rows.map(async (row) =>
        mapSharedInvestigationRow(row as DbRow, await this.getSharedInvestigationSteps(String(row.id))),
      ),
    );
  }
  ...
  async getDecisionBrief(id: string) {
    const query = await this.pool.query(`select * from decision_briefs where id = $1 limit 1`, [id]);
    if (!query.rowCount) {
      return null;
    }
    return mapDecisionBriefRow(query.rows[0] as DbRow);
  ...
  async listDecisionBriefs(options: {
    ...
    return query.rows.map((row) => mapDecisionBriefRow(row as DbRow));
  }
  ...
  async listDecisionCheckpoints(options: { decision_brief_id: string; limit?: number }) {
    ...
    return query.rows.map((row) => mapDecisionCheckpointRow(row as DbRow));
  }
  ...
  async getPortfolioCandidate(id: string) {
    const query = await this.pool.query(`select * from portfolio_candidates where id = $1 limit 1`, [id]);
    if (!query.rowCount) {
      return null;
    }
    return mapPortfolioCandidateRow(query.rows[0] as DbRow);
  ...
  async listPortfolioCandidates(options: {
    ...
    return query.rows.map((row) => mapPortfolioCandidateRow(row as DbRow));
  }
  ...
  async listPortfolioCheckpoints(options: { portfolio_candidate_id: string; limit?: number }) {
    ...
    return query.rows.map((row) => mapPortfolioCheckpointRow(row as DbRow));
  }
  ...
  async getPortfolioReviewSession(id: string) {
    const query = await this.pool.query(`select * from portfolio_review_sessions where id = $1 limit 1`, [id]);
    if (!query.rowCount) {
      return null;
    }
    return mapPortfolioReviewSessionRow(query.rows[0] as DbRow);
  ...
  async listPortfolioReviewSessions(options: {
    ...
    return query.rows.map((row) => mapPortfolioReviewSessionRow(row as DbRow));
  }
  ...
  async listPortfolioReviewSessionItems(options: { review_session_id: string }) {
    ...
    return query.rows.map((row) => mapPortfolioReviewSessionItemRow(row as DbRow));
  }
  ...
  async listPortfolioRebalanceProposals(options: {
    ...
    return query.rows.map((row) => mapPortfolioRebalanceProposalRow(row as DbRow));
  }
  ...
  async listWorkspaceRecentItems(options: { workspace_id: string; limit?: number }) {
    ...
    return query.rows.map((row) => mapWorkspaceRecentItemRow(row as DbRow));
  }
  ...
  async listWorkspaceActivity(options: { workspace_id: string; limit?: number }) {
    ...
    return query.rows.map((row) => mapWorkspaceActivityRow(row as DbRow));
  }
  ...
  async getSharedReviewNote(input: { workspace_id: string; prediction_id: string }) {
    ...
    return query.rowCount ? mapSharedReviewNoteRow(query.rows[0] as DbRow) : null;
  }
  ...
  async reset() {
    ...
    `);
    await this.pool.query(await loadPhase1SchemaSql());
  }
