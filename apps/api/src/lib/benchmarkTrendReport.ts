import { benchmarkTrendReportSchema } from "@finance-superbrain/schemas";
import type {
...
const bestFamily = (
  families: BenchmarkTrendReport["families"],
  selector: (family: BenchmarkTrendReport["families"][number]) => number,
) => {
  if (!families.length) {
    return null;
  }
...
export const buildBenchmarkTrendReport = async (
  ...
  return buildBenchmarkTrendReportFromSnapshots(benchmarkPackId, snapshots);
};

export const buildBenchmarkTrendReportFromSnapshots = (
  ...
  const families: BenchmarkTrendReport["families"] = [];

  for (const familyName of familyNames) {
    const points: BenchmarkTrendReport["families"][number]["snapshots"] = [];
  ...
  return benchmarkTrendReportSchema.parse({
    generated_at: new Date().toISOString(),
    benchmark_pack_id: benchmarkPackId,
...
export const latestBenchmarkSnapshotForPack = (
  snapshots: BenchmarkReplaySnapshot[],
  benchmarkPackId: string,
) =>
  [...snapshots]
