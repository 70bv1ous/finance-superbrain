...
export type FomcTrackedOutcome = {
...
export type FomcOutcomeResult = {
  tracked: FomcTrackedOutcome[];
...
export const trackFomcOutcome = (input: FomcOutcomeInput): FomcOutcomeResult => {
  const { prediction_result, realized_moves, measured_at, timing_alignment, dominant_catalyst } =
    input;

  const tracked: FomcTrackedOutcome[] = prediction_result.predictions.map((prediction) => {
