...
export const trackCpiOutcome = (input: CpiOutcomeInput): CpiOutcomeResult => {
  const { prediction_result, realized_moves, measured_at, timing_alignment, dominant_catalyst } =
    input;

  const tracked: TrackedOutcome[] = prediction_result.predictions.map((prediction) => {
