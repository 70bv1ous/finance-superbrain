...
import type { NfpAnalogMatch } from "./nfpAnalogRetrieval.js";

// ─── Types ────────────────────────────────────────────────────────────────────

export type NfpEnrichedPrediction = GeneratedPrediction & {
  analog_count: number;
  analog_boost: number;
};

export type NfpEnrichedPredictionResult = Omit<NfpPredictionResult, "predictions"> & {
  predictions: NfpEnrichedPrediction[];
  analogs: NfpAnalogMatch[];
};
...
/**
 * Compute a net confidence delta from the NFP analog set.
 * Mirrors CPI / FOMC: reinforcements push confidence up, mistakes push down,
 * scaled by average similarity.  Maximum swing is ±0.10.
 */
const computeAnalogBoost = (analogs: NfpAnalogMatch[]): number => {
...
const buildAnalogEvidenceLine = (analogs: NfpAnalogMatch[]): string => {
  ...
  return (
    `${analogs.length} NFP analog(s): ${correct} correct, ${partial} partial, ` +
    `${wrong} wrong. Top match: ${top.period} (similarity ${top.similarity}).`
  );
};

const buildCautionLine = (analogs: NfpAnalogMatch[]): string | null => {
...
/**
 * Enrich an NfpPredictionResult with signals from retrieved NFP analogs.
 * Mirrors `enrichFomcPredictionWithAnalogs` exactly.
 * All confidence values stay within [0.35, 0.95].
 */
export const enrichNfpPredictionWithAnalogs = (
  result: NfpPredictionResult,
  analogs: NfpAnalogMatch[],
  ...
  const enrichedPredictions: NfpEnrichedPrediction[] = result.predictions.map(
