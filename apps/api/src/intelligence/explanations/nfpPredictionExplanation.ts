...
import type { NfpKnowledgeEnrichedResult } from "../reliability/nfpKnowledgeEnrichment.js";

// ─── Types ────────────────────────────────────────────────────────────────────

export type NfpAdjustmentBreakdown = {
...
export type NfpPredictionExplanation = {
  horizon: string;
  cluster_id: string;
  surprise_direction: string;
  analog_count: number;
  confidence_breakdown: NfpAdjustmentBreakdown;
  evidence: NfpEvidenceItem[];
  cautions: NfpEvidenceItem[];
  supports: NfpEvidenceItem[];
...
const buildAnalogEvidenceItems = (
  result: NfpKnowledgeEnrichedResult,
  analogBoost: number,
): NfpEvidenceItem[] => {
    ...
    return [
      {
        ...
        signal: "neutral",
        label: "no_analogs",
        description: "No historical NFP analog cases found for this macro setup.",
      },
    ];
  }
...
export const buildNfpPredictionExplanation = (
  result: NfpKnowledgeEnrichedResult,
  horizon: string,
  ...
  if (!pred) {
    throw new Error(
      `buildNfpPredictionExplanation: no prediction found for horizon "${horizon}". ` +
        `Available: ${result.predictions.map((p) => p.horizon).join(", ")}.`,
    );
  }

  const themeKey = resolveThemeKeyFromPrediction(result.nfp_event, result.context);
  ...
  const confidenceBreakdown: NfpAdjustmentBreakdown = {
  ...
  return {
    ...
    surprise_direction: result.nfp_event.surprise_direction,
...
export const buildNfpPredictionExplanations = (
  result: NfpKnowledgeEnrichedResult,
): NfpPredictionExplanation[] =>
  result.predictions.map((pred) =>
    buildNfpPredictionExplanation(result, pred.horizon),
  );
